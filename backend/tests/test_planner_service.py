...
def test_build_weekly_plan_returns_seven_days_and_summary():
    ...
    result = build_weekly_plan(pantry, region="south_indian", household_size=3, days=7)

    assert len(result["days"]) == 7
    assert result["profile"]["id"] == "south_indian"
...
def test_build_weekly_plan_clamps_days_and_household_size():
    result = build_weekly_plan([], region="punjabi", household_size=99, days=99)
...
def test_cuisine_profiles_include_requested_regions():
    ...
    assert {"south_indian", "bengali", "punjabi"}.issubset(profile_ids)
...
def test_delivery_partners_create_blinkit_and_zepto_links():
    partners = get_delivery_partners(["rice", "paneer"])

    partner_ids = {partner["id"] for partner in partners}

    assert partner_ids == {"blinkit", "zepto"}
