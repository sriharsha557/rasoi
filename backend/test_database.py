...
async def test_database():
    ...
    print(f"[OK] Created item: {created_item['name']} (ID: {created_item['id'][:8]}...)")
    ...
    print(f"[OK] Retrieved item by ID: {retrieved_item['name']}")
    
    # Test GET_ALL
    print("\n4. Testing GET_ALL operation...")
    all_items = await repo.get_all()
    print(f"[OK] Retrieved {len(all_items)} item(s) from pantry")
    ...
    print(f"[OK] Created: Milk (expires in 5 days)")
    ...
    print(f"[OK] Updated quantity: {created_item['quantity']} -> {updated_item['quantity']}")
    ...
    print("[OK] Trigger for updated_at is configured")
    ...
    print(f"[OK] Deleted item: Bread")
    
    remaining_items = await repo.get_all()
    print(f"[OK] Remaining items: {len(remaining_items)}")
    ...
    print("[OK] Returns False for non-existent item")
    ...
    print("[OK] Returns None for non-existent item")
    ...
    try:
        ...
        print(f"[OK] Correctly rejected negative quantity: {type(e).__name__}")
