    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recipeState.recipes.map((recipe) => (
          ...
          >
            {(recipe.imageUrl ?? recipe.image_url ?? recipe.image) && (
              <img
            ...
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-6 text-white relative">
              ...
              <h3 className="text-xl font-bold mb-2 pr-24">{recipe.title ?? recipe.name}</h3>
              ...
              <div className="flex flex-wrap items-center gap-2 text-blue-100">
                ...
                {recipe.cuisine && <span className="text-sm">{recipe.cuisine}</span>}
                {(recipe.mealType ?? recipe.meal_type) && <span className="text-sm">{recipe.mealType ?? recipe.meal_type}</span>}
                {recipe.diet && <span className="text-sm">{recipe.diet}</span>}
                {(recipe.caloriesKcal ?? recipe.calories_kcal) !== null && (recipe.caloriesKcal ?? recipe.calories_kcal) !== undefined && (
                  <span className="text-sm">{Math.round(recipe.caloriesKcal ?? recipe.calories_kcal ?? 0)} kcal</span>
                )}
              </div>
            </div>
